...
from src.agency.pomdp import Action, HiddenState, Observation
...
def build_generative_model():
    ...
    A[Observation.GOAL_SIGNAL_ON.value - 1, HiddenState.KNOWLEDGE_HIGH.value - 1] = (
        0.1  # Small chance of spontaneous success
    )
    ...
    B[:, :, u_sb] = 0  # Reset
    B[HiddenState.KNOWLEDGE_MEDIUM.value - 1, HiddenState.KNOWLEDGE_LOW.value - 1, u_sb] = (
        0.1  # Hard (10%)
    )
    B[HiddenState.KNOWLEDGE_LOW.value - 1, HiddenState.KNOWLEDGE_LOW.value - 1, u_sb] = (
        0.9  # Likely stay low
    )
    # Med -> High
    ...
    B[:, :, u_ss] = 0
    B[HiddenState.KNOWLEDGE_MEDIUM.value - 1, HiddenState.KNOWLEDGE_LOW.value - 1, u_ss] = (
        0.6  # Good chance (60%)
    )
    B[HiddenState.KNOWLEDGE_LOW.value - 1, HiddenState.KNOWLEDGE_LOW.value - 1, u_ss] = 0.4
    # Med -> High
    B[HiddenState.KNOWLEDGE_HIGH.value - 1, HiddenState.KNOWLEDGE_MEDIUM.value - 1, u_ss] = (
        0.8  # Very effective when narrowed
    )
    ...
    C[Observation.FINDING_RELEVANT.value - 1] = (
        1.0  # Epistemic value implicitly handled, but slight pref for relevance
    )
