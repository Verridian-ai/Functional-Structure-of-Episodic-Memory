...
from src.logic.gsw_schema import Actor, ChunkExtraction, GlobalWorkspace, PredictiveQuestion
...
class LegalReconciler:
    ...
    def __init__(
        ...
        self.entity_matcher = EntityMatcher(
            client=self.client, model=self.model, use_toon=self.use_toon
        )
    ...
    def reconcile(
        self, new_extraction: ChunkExtraction, workspace: GlobalWorkspace, chunk_text: str
        ...
        entity_matches = self.entity_matcher.reconcile_entities(
            new_extraction.actors, workspace, chunk_text
        )
        ...
        for match in entity_matches:
            ...
            # Find the new actor
            new_actor = next((a for a in new_extraction.actors if a.id == new_id), None)
        ...
        answered = self.question_answerer.answer_questions(
            workspace.get_unanswered_questions(), chunk_text, new_extraction
        )
        ...
        for actor in new_extraction.actors:
            ...
            was_matched = any(m["new_entity_id"] == actor.id for m in entity_matches)

            if not was_matched:
                workspace.add_actor(actor)
                reconciliation_log.append(
                    {"action": "added_new", "entity_id": actor.id, "name": actor.name}
                )
    ...
    def _merge_actors(self, existing: Actor, new: Actor) -> None:
        ...
        for state in new.states:
            ...
            existing_state = next(
                (s for s in existing.states if s.name == state.name and s.value == state.value),
                None,
            )
    ...
    def _update_references(self, extraction: ChunkExtraction, old_id: str, new_id: str) -> None:
        ...
        for verb in extraction.verb_phrases:
            ...
            verb.patient_ids = [new_id if pid == old_id else pid for pid in verb.patient_ids]
        ...
        for link in extraction.spatio_temporal_links:
            link.linked_entity_ids = [
                new_id if eid == old_id else eid for eid in link.linked_entity_ids
            ]
