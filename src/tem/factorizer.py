...
from src.logic.gsw_schema import Actor, ActorType, ChunkExtraction, GlobalWorkspace, State
from src.tem.structures import CaseArchetype, CaseStructure
...
class HeuristicTEMFactorizer:
    ...
    def factorize(self, data: GlobalWorkspace | ChunkExtraction) -> CaseStructure:
        """
        ...
        return CaseStructure(
            archetype=archetype, confidence=confidence, features=features, reasoning=reasoning
        )
    ...
    def _classify_archetype(self, features: dict[str, float]) -> tuple[CaseArchetype, float, str]:
        ...
        if features["has_property"] > 0 or features["financial_disparity"] > 0:
            ...
            if (
                features["relationship_duration_years"] > 0
                and features["relationship_duration_years"] < 5
            ):
                return (
            ...
            return (CaseArchetype.PROPERTY_HIGH_WEALTH, 0.5, "Property matter detected.")
