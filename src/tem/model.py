...
class SensoryModule(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(input_dim, hidden_dim * 2), nn.ReLU(), nn.Linear(hidden_dim * 2, hidden_dim)
        )
        self.decoder = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim * 2), nn.ReLU(), nn.Linear(hidden_dim * 2, input_dim)
        )
...
class MemoryModule(nn.Module):
    ...
    def update(
        self, M: torch.Tensor, g: torch.Tensor, p: torch.Tensor, eta: float = 0.1
    ) -> torch.Tensor:
        """
