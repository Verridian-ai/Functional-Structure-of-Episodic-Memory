...
from src.tem.action_space import ACTION_TO_ID, LegalAction
...
class LegalGraphBuilder:
    ...
    def _load_graph(self):
        ...
        if edges_path.exists():
            with open(edges_path, encoding="utf-8") as f:
                ...
                for row in rows:
                    ...
                    target = row["target"]
                    rel_type = row.get(
                        "action", row.get("type", "CITE")
                    )  # Support both 'action' and 'type' field
        ...
        print(
            f"Graph loaded: {len(self.nodes)} nodes, {sum(len(v) for v in self.adj_list.values())} edges."
        )
    ...
    def batch_generator(
        self, batch_size: int, walk_length: int
