...
try:
    from src.retrieval.retriever import LegalRetriever, SimpleBM25
...
class SearchMethodsBenchmark:
    ...
    def _initialize_retrievers(self) -> None:
        ...
        if LegalRetriever is not None:
            try:
                self.retrievers["legal_bm25"] = LegalRetriever(
                    data_dir=str(self.data_dir), enable_vsa_validation=False  # Disable for speed
                )
                print(
                    f"  - LegalRetriever: initialized ({self.retrievers['legal_bm25'].bm25.corpus_size} docs)"
                )
            except Exception as e:
        ...
        if GSWRetriever is not None:
            try:
                self.retrievers["gsw"] = GSWRetriever(workspace_dir=self.workspace_dir)
                stats = self.retrievers["gsw"].get_statistics()
                print(
                    f"  - GSWRetriever: initialized ({stats['total_actors']} actors, "
        ...
        if HybridRetriever is not None:
            try:
                self.retrievers["hybrid"] = HybridRetriever(
                    workspace_dir=self.workspace_dir, data_dir=self.data_dir
                )
        ...
        if HybridRetriever is not None:
            try:
                self.retrievers["hybrid_blend"] = HybridRetriever(
                    workspace_dir=self.workspace_dir, data_dir=self.data_dir, blend_results=True
                )
    ...
    def _execute_query(self, method: str, query: BenchmarkQuery, top_k: int = 5) -> QueryResult:
        """
        ...
        if retriever is None:
            return QueryResult(
                query_id=query.query_id, method=method, results=[], latency_ms=0.0, result_count=0
            )
        ...
        try:
            if method == "legal_bm25":
                ...
                results = retriever.retrieve(
                    ...
                    mode="auto",
                    domain=(
                        query.domain if query.domain in retriever.gsw_retriever.workspaces else None
                    ),
                )
            else:
                results = []
    ...
    def _calculate_precision(self, result: QueryResult, query: BenchmarkQuery) -> float:
        """
        ...
        for res in result.results:
            ...
            if "states" in res and res["states"]:
                for state in res["states"]:
                    text_to_check += (
                        f"{state.get('name', '')} {state.get('value', '')}".lower() + " "
                    )
    ...
    def _calculate_recall(self, result: QueryResult, query: BenchmarkQuery) -> float:
        """
    ...
    def _calculate_mrr(self, result: QueryResult, query: BenchmarkQuery) -> float:
        """
    ...
    def _aggregate_metrics(
        self, method: str, results: list[QueryResult], queries: list[BenchmarkQuery]
    ) -> MethodMetrics:
        """
    ...
    def _generate_recommendations(self, method_metrics: dict[str, MethodMetrics]) -> list[str]:
        ...
        best_f1_method = max(method_metrics.items(), key=lambda x: x[1].f1_score)[0]
    ...
    def run_full_benchmark(self, top_k: int = 5, save_raw_results: bool = True) -> BenchmarkReport:
        """
        ...
        if self.tracker is not None:
            self.tracker.record_suite_run(
                ...
                overall_score=(
                    statistics.mean([m.f1_score for m in method_metrics.values()])
                    if method_metrics
                    else 0.0
                ),
                status="completed",
    ...
    def _generate_comparison(self, method_metrics: dict[str, MethodMetrics]) -> dict[str, Any]:
        ...
        # Ranking by F1
        ranked = sorted(method_metrics.items(), key=lambda x: x[1].f1_score, reverse=True)
        ...
        comparison["ranking_by_latency"] = [
            {"method": m, "avg_latency_ms": met.avg_latency_ms} for m, met in ranked_latency
        ]
        ...
        for category in all_categories:
            ...
            for method, met in method_metrics.items():
                if category in met.by_category:
                    ...
                    f1 = (
                        2
                        * (cat_data["precision"] * cat_data["recall"])
                        / (cat_data["precision"] + cat_data["recall"])
                        if (cat_data["precision"] + cat_data["recall"]) > 0
                        else 0
                    )
                    if f1 > best_f1:
                        best_f1 = f1
                        best_method = method
            if best_method:
                comparison["best_by_category"][category] = {"method": best_method, "f1": best_f1}
        ...
        for difficulty in all_difficulties:
            ...
            for method, met in method_metrics.items():
                if difficulty in met.by_difficulty:
                    ...
                    f1 = (
                        2
                        * (diff_data["precision"] * diff_data["recall"])
                        / (diff_data["precision"] + diff_data["recall"])
                        if (diff_data["precision"] + diff_data["recall"]) > 0
                        else 0
                    )
                    if f1 > best_f1:
                        best_f1 = f1
    ...
    def save_report(self, report: BenchmarkReport, output_path: Path) -> None:
        ...
        report_dict = {
            ...
            "method_metrics": {
                method: asdict(metrics) for method, metrics in report.method_metrics.items()
            },
    ...
    def print_summary(self, report: BenchmarkReport) -> None:
        ...
        for method in report.methods_tested:
            ...
            print(
                f"{method:<20} {metrics.precision:>10.3f} {metrics.recall:>10.3f} "
        ...
        for method in report.methods_tested:
            metrics = report.method_metrics[method]
            print(
                f"{method:<20} {metrics.avg_latency_ms:>10.1f} {metrics.p50_latency_ms:>10.1f} "
...
def main():
    ...
    try:
        ...
        # Save report
        benchmark.save_report(report, Path("data/benchmarks/search_methods_benchmark_report.json"))
