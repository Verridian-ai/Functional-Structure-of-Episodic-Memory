...
from .schema import Entity, Event, LegalCase, State
...
class Reconciler:
    ...
    def _merge_entities(self, new_entities: dict[str, Entity], global_store: dict[str, Entity]):
        ...
        for new_id, new_entity in new_entities.items():
            ...
            if best_score > SIMILARITY_THRESHOLD:
                ...
                print(
                    f"Reconciler: Merged '{new_entity.name}' into '{existing_entity.name}' (Score: {best_score:.4f})"
                )
    ...
    def _merge_states(self, new_states: list[State]):
        ...
        for new_state in new_states:
            ...
            existing_states = [
                s
                for s in self.global_case.states
            ...
            open_state = next(
                (s for s in existing_states if s.is_ongoing or s.end_date is None), None
            )

            if open_state:
                if (
                    new_state.start_date
                    and open_state.start_date
                    and new_state.start_date > open_state.start_date
                ):
                    ...
                    print(
                        f"Reconciler: Updated State '{new_state.name}' for {new_state.entity_id}: Closed previous state, opened new."
                    )
