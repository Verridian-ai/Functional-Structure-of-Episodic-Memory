...
class LegalRetriever:
    ...
    def retrieve_with_validation(
        ...
        result_dict = {"results": results, "query": query, "num_results": len(results)}
        ...
        if generate_response:
            ...
            if workspace and self.vsa_validator:
                validation = self.vsa_validator.validate_response(query, response, workspace)
                ...
                if validation["severity"]["high_risk"] > 0:
                    result_dict["warning"] = (
                        f"High risk: {validation['severity']['high_risk']} claims flagged"
                    )
                elif validation["severity"]["medium_risk"] > 0:
                    result_dict["warning"] = (
                        f"Medium risk: {validation['severity']['medium_risk']} claims need review"
                    )
    ...
    def validate_claim(self, claim: str, workspace: GlobalWorkspace) -> dict:
        ...
        if not self.vsa_validator:
            return {"error": "VSA validation is not enabled", "valid": None}

        return self.vsa_validator.validate_claim(claim, workspace)

    def batch_validate_claims(self, claims: list[str], workspace: GlobalWorkspace) -> list[dict]:
        """
        ...
        """
        if not self.vsa_validator:
            return [{"error": "VSA validation is not enabled", "valid": None} for _ in claims]

        return [self.vsa_validator.validate_claim(claim, workspace) for claim in claims]
