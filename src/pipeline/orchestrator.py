...
class FullPipeline:
    ...
    def run(self, stages: list[str] | None = None, resume: bool = False) -> PipelineState:
        """
        Run pipeline stages.
        ...
        print(f"\n{'='*70}")
        print("  LEGAL CORPUS PROCESSING PIPELINE")
        print(f"{'='*70}\n")
    ...
    def _run_classification(self) -> StageResult:
        ...
        if self.config.enable_auto_gsw:
            ...
            print(
                f"[Classification] GSW auto-queue enabled (threshold={self.config.gsw_authority_threshold})"
            )
        ...
        try:
            ...
            return StageResult(stage="classify", success=False, duration_seconds=0, errors=[str(e)])
                        ...
                        if authority >= self.config.gsw_authority_threshold:
                            ...
                            doc = {
                                ...
                                "_classification": {
                                    ...
                                    "case_refs": (
                                        row.get("case_refs", "").split("|")
                                        if row.get("case_refs")
                                        else []
                                    ),
                                    "legislation_refs": (
                                        row.get("leg_refs", "").split("|")
                                        if row.get("leg_refs")
                                        else []
                                    ),
                                },
    ...
    def _run_gsw_extraction(self) -> StageResult:
        ...
        if not self.config.enable_auto_gsw:
            ...
            return StageResult(
                stage="gsw", success=True, duration_seconds=0, statistics={"skipped": True}
            )
        ...
        if self.gsw_queue.empty():
            ...
            return StageResult(
                stage="gsw", success=True, duration_seconds=0, statistics={"documents_processed": 0}
            )
            ...
            while not self.gsw_queue.empty():
                ...
                for doc in batch:
                    try:
                        ...
                        if domain not in domain_workspaces:
                            ...
                            if workspace_path.exists():
                                ...
                                manager = WorkspaceManager(
                                    workspace=workspace, storage_path=workspace_path
                                )
                        ...
                        error_msg = (
                            f"Error processing {doc.get('citation', 'unknown')}: {str(e)[:100]}"
                        )
    ...
    def _run_graph_building(self) -> StageResult:
        ...
        if not self.config.enable_graph_building:
            ...
            return StageResult(
                stage="graph", success=True, duration_seconds=0, statistics={"skipped": True}
            )

        from src.graph.spcnet_builder import SPCNetBuilder

        print("[Graph] Building citation network...")
        ...
        try:
            ...
            builder = SPCNetBuilder(
                input_dir=self.config.output_dir / "cases", output_dir=self.config.graph_dir
            )
            ...
            return StageResult(stage="graph", success=False, duration_seconds=0, errors=[str(e)])
    ...
    def _run_visualization(self) -> StageResult:
        ...
        try:
            ...
            return StageResult(
                stage="viz", success=False, duration_seconds=0, errors=[f"Import error: {e}"]
            )
            ...
            traceback.print_exc()
            return StageResult(stage="viz", success=False, duration_seconds=0, errors=[str(e)])
    ...
    def _print_final_summary(self) -> None:
        ...
        for stage, result in self.state.stage_results.items():
            ...
            print(
                f"  [{status}] {stage.upper()}: {result.duration_seconds:.1f}s, {result.documents_processed} docs"
            )
            if result.errors:
...
def run_pipeline(
    config: PipelineConfig | None = None, stages: list[str] | None = None, resume: bool = False
) -> PipelineState:
    """
