...
from src.logic.ontology_seed import STANDARD_ASSETS, STANDARD_EVENTS, STANDARD_OUTCOMES
from src.logic.schema import Entity, LegalCase, State
...
class Reconciler:
    ...
    def __init__(self, vector_store: LocalVectorStore):
        ...
        self.global_graph = LegalCase(
            case_id="GLOBAL_PILOT", title="Australian Family Law Knowledge Graph"
        )
    ...
    def ingest_chunk(self, new_case: LegalCase):
        ...
        self._reconcile_entities(
            new_case.persons, self.global_graph.persons, "Person", new_case.case_id
        )

        # 2. Reconcile Objects
        self._reconcile_entities(
            new_case.objects, self.global_graph.objects, "Object", new_case.case_id
        )
    ...
    def get_current_context(self) -> str:
        ...
        for term, count in asset_counts.items():
            if count > 3:
                active_assets.add(term)

        for term, count in outcome_counts.items():
            if count > 3:
                active_outcomes.add(term)

        for term, count in event_counts.items():
            if count > 3:
                active_events.add(term)

        # 3. Get Sample People
        sorted_people = sorted(
            self.global_graph.persons, key=lambda p: len(p.involved_cases), reverse=True
        )
    ...
    def load_graph(self, filepath: str):
        ...
        try:
            ...
            print(
                f"[Reconciler] Loaded Global Graph from {path} ({len(self.global_graph.persons)} persons)."
            )
        ...
        for entity in new_entities:
            ...
            if match_id:
                ...
                if global_entity:
                    ...
                    if entity.description and len(entity.description) > len(
                        global_entity.description or ""
                    ):
    ...
    def _reconcile_states(self, new_states: list[State]):
        ...
        for new_state in new_states:
            ...
            existing = [
                s
                for s in self.global_graph.states
