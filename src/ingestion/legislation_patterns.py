...
PART_PATTERN = re.compile(r"\bPart\s+([IVXLCDM]+|\d+[A-Z]?)", re.IGNORECASE)

# Pattern to match Division references
DIVISION_PATTERN = re.compile(r"\bDivision\s+(\d+[A-Z]?)", re.IGNORECASE)

# Pattern to match Schedule references
SCHEDULE_PATTERN = re.compile(r"\bSchedule\s+(\d+[A-Z]?)", re.IGNORECASE)
    ...
    if not results:
        for domain, keywords in LEGISLATION_TITLE_PATTERNS.items():
            for keyword in keywords:
                ...
                pattern = re.compile(
                    r"\b" + re.escape(keyword) + r".*?\b(Act|Regulations|Rules|Order)\b",
                    re.IGNORECASE,
                )
...
def get_domain_for_legislation(act_name: str) -> str | None:
...
for act_name in LEGISLATION_TO_DOMAIN:
