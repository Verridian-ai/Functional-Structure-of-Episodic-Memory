...
MEDIUM_NEUTRAL_PATTERN = re.compile(r"\[(\d{4})\]\s*([A-Z]{2,10})\s*(\d+)", re.IGNORECASE)

# Authorized Report: (2020) 271 CLR 657, [2020] 1 AC 432
AUTHORIZED_REPORT_PATTERN = re.compile(r"\((\d{4})\)\s+(\d+)\s+([A-Z]{2,6})\s+(\d+)", re.IGNORECASE)
...
def get_domain_for_case(case_name: str) -> str | None:
