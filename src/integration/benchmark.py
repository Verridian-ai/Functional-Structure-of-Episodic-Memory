    ...
    def run(self):
        ...
        for test in tests:
            ...
            if test["expected_logic_validation"]:
                ...
                if not success or (
                    output["status"] == "success" and "Validation FAILED" in str(output)
                ):
            ...
            if logic_correct:
                results["logic_checks_correct"] += 1
            if (
                success
                and test["expected_logic_validation"]
                or not success
                and not test["expected_logic_validation"]
            ):
                results["passed"] += 1
