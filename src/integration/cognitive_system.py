...
from src.agency.pomdp import Action, Observation
    ...
    def process_query(self, query: str, max_steps: int = 10) -> dict[str, Any]:
        ...
        for t in range(max_steps):
            ...
            if action == Action.STOP:
                ...
                # 1. From Query
                if "divorce" in query.lower():
                    concepts.append("DIVORCE")
                if "separate" in query.lower() or "separation" in query.lower():
                    concepts.append("SEPARATION")
                if "child" in query.lower():
                    concepts.append("CHILD")
                if "property" in query.lower():
                    concepts.append("PROPERTY_SETTLEMENT")
                if "violence" in query.lower():
                    concepts.append("FAMILY_VIOLENCE")

                # 2. From Retrieved Text (Simple Keyword Extraction)
                all_text = " ".join(self.gathered_facts.entity_summaries.values()).lower()

                if "marriage" in all_text or "married" in all_text:
                    concepts.append("MARRIAGE")
                if "asset" in all_text or "pool" in all_text or "home" in all_text:
                    concepts.append("ASSET")
                if "separation" in all_text or "separated" in all_text:
                    concepts.append("SEPARATION")
                if "child" in all_text or "parenting" in all_text:
                    concepts.append("CHILD")
                if "violence" in all_text or "abuse" in all_text:
                    concepts.append("FAMILY_VIOLENCE")
                if "risk" in all_text:
                    concepts.append("SAFETY_RISK")
                ...
                if results:
                    ...
                    for doc in results:
                        print(
                            f" - {doc.get('title', 'Untitled')} (Score: {doc.get('score', 0):.2f})"
                        )
                        # Add to gathered facts (simulate reading)
                        self.gathered_facts.entity_summaries[doc["id"]] = doc.get(
                            "text_preview", ""
                        )
                ...
                if len(self.gathered_facts.entity_summaries) > 0:
                    print(
                        f"[System] Reading {len(self.gathered_facts.entity_summaries)} cached documents..."
                    )
