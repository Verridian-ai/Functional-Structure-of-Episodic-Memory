...
function SettingsTab({
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* Security Warning */}
      <div className="flex items-start gap-2 sm:gap-3 p-3 sm:p-4 bg-amber-900/20 border border-amber-700/50 rounded-lg sm:rounded-xl">
      ...
      <div>
        ...
        >
          <Key className="w-3.5 h-3.5 sm:w-4 sm:h-4 inline mr-1.5 sm:mr-2" aria-hidden="true" />
          OpenRouter API Key
        </label>
        <div className="relative">
          <input
            ...
            className={`w-full p-2.5 sm:p-3 pr-12 bg-zinc-800 border rounded-lg sm:rounded-xl text-sm sm:text-base text-white placeholder:text-zinc-500 focus:outline-none transition ${
              apiKeyError
          ...
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-zinc-700 active:bg-zinc-700 rounded transition touch-target"
            aria-label={showApiKey ? 'Hide API key' : 'Show API key'}
          >
            {showApiKey ? (
              <EyeOff className="w-4 h-4 sm:w-5 sm:h-5 text-zinc-400" />
            ) : (
        ...
        {apiKeyError && (
          <p id="api-key-error" className="flex items-center gap-1 text-[10px] sm:text-xs text-red-400 mt-1.5 sm:mt-2" role="alert">
            <AlertTriangle className="w-3 h-3" />
            {apiKeyError}
          </p>
        )}
        <p id="api-key-help" className="text-[10px] sm:text-xs text-zinc-500 mt-1.5 sm:mt-2">
          Get your API key from{' '}
          <a
            href="https://openrouter.ai/keys"
            target="_blank"
            rel="noopener noreferrer"
            className="text-cyan-400 hover:underline active:underline"
          >
            openrouter.ai/keys
          </a>
        </p>
